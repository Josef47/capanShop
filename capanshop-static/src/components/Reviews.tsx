  ...
  return (
    <div className="flex gap-1 text-gold-400" aria-label={`${rating}/5`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <span key={i} className={i < rating ? "" : "opacity-25"}>★</span>
      ))}
    </div>
  );
}
          ...
          {REVIEWS.map((r, i) => (
            <Reveal key={r.id} delay={i * 100}>
              <figure className="card relative h-full p-7 transition-all duration-500 hover:-translate-y-1 hover:border-gold-400/40">
                <span className="absolute -top-4 left-6 font-display text-6xl leading-none text-gold-400/30">&ldquo;</span>
                <Stars rating={r.rating} />
                <blockquote className="mt-4 text-sm leading-relaxed text-cream-200">{r.text[lang]}</blockquote>
