    ...
    <footer className="border-t border-gold-400/15 bg-coffee-950 py-12">
      <div className="container-shop flex flex-col items-center gap-6 px-4 text-center sm:px-6 lg:px-8">
        ...
        <div className="flex gap-6 text-sm text-cream-300">
          <a href={`tel:${SHOP.phone.replace(/\s/g, "")}`} className="transition-colors hover:text-gold-300">{SHOP.phoneDisplay}</a>
          <span className="text-coffee-700">|</span>
          <span>{SHOP.address}</span>
        </div>
        <a href={SHOP.instagramUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-cream-300 transition-colors hover:text-gold-300">
